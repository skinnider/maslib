...
write_sh = function(job_name,
  ...
  if (current_system == 'sockeye') {
    ...
    env_lines = c(
      '# >>> conda initialize >>>',
      '# !! Contents within this block are managed by \'conda init\' !!',
      '__conda_setup="$(\'/scratch/gfps/SKINNIDER/miniconda3/bin/conda\' \'shell.bash\' \'hook\' 2> /dev/null)"',
      'if [ $? -eq 0 ]; then',
      'eval "$__conda_setup"',
      'else',
      'if [ -f "/scratch/gfps/SKINNIDER/miniconda3/etc/profile.d/conda.sh" ]; then',
      '. "/scratch/gfps/SKINNIDER/miniconda3/etc/profile.d/conda.sh"',
      'else',
      'export PATH="/scratch/gfps/SKINNIDER/miniconda3/bin:$PATH"',
      'fi',
      'fi',
      'unset __conda_setup',
      '# <<< conda initialize <<<',
