...
write_sh = function(job_name,
  ...
  if (current_system == 'sockeye') {
    ...
    env_lines = c(
      ...
      '__conda_setup="$(\'/arc/project/st-ljfoster-1/miniconda3/bin/conda\' \'shell.bash\' \'hook\' 2> /dev/null)"',
      'if [ $? -eq 0 ]; then',
      'eval "$__conda_setup"',
      'else',
      'if [ -f "/arc/project/st-ljfoster-1/miniconda3/etc/profile.d/conda.sh" ]; then',
      '. "/arc/project/st-ljfoster-1/miniconda3/etc/profile.d/conda.sh"',
      'else',
      'export PATH="/arc/project/st-ljfoster-1/miniconda3/bin:$PATH"',
      'fi',
      'fi',
      'unset __conda_setup',
      '# <<< conda initialize <<<',
    ...
    header_lines = c(
      ...
      ifelse(gpu,
             ifelse(!is.na(gpu_spec),
                    ...
                    paste0('#SBATCH --gres=gpu:1')),
             ''),
      paste0('#SBATCH --partition=main,skinniderlab'),
      ''
    )
    ...
    env_lines = c(
      '# >>> conda initialize >>>',
      '# !! Contents within this block are managed by \'conda init\' !!',
      '__conda_setup="$(\'/scratch/gpfs/SKINNIDER/miniconda3/bin/conda\' \'shell.bash\' \'hook\' 2> /dev/null)"',
      'if [ $? -eq 0 ]; then',
      'eval "$__conda_setup"',
      'else',
      'if [ -f "/scratch/gpfs/SKINNIDER/miniconda3/etc/profile.d/conda.sh" ]; then',
      '. "/scratch/gpfs/SKINNIDER/miniconda3/etc/profile.d/conda.sh"',
      'else',
      'export PATH="/scratch/gpfs/SKINNIDER/miniconda3/bin:$PATH"',
      'fi',
      'fi',
      'unset __conda_setup',
      '# <<< conda initialize <<<',
